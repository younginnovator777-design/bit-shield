...
| Component | URL |
|---|---|
| Frontend App | https://bit-shield.vercel.app |
| Backend API | https://bit-shield.onrender.com |
| API Documentation | https://bit-shield.onrender.com/docs |
...
Create a `.env.local` file inside the `frontend/` directory to point your local frontend to the live Render backend:
 
```env
NEXT_PUBLIC_API_URL=https://bit-shield.onrender.com
```
 
> **Note:** This file is environment-specific and should not be committed to version control — make sure `.env.local` is listed in your `.gitignore`.
...
| Issue | Resolution |
|---|---|
| `ModuleNotFoundError` | Verify that your virtual environment is active (`(.venv)` prefix in terminal) and that `PYTHONPATH=.` is included when running Python scripts manually. |
